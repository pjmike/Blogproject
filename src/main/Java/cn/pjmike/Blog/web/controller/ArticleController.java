...
public class ArticleController {
    ...
    @ApiOperation(value = "获取文章带评论",notes = "某一文章",response = ResponseResult.class)
    public ResponseResult<ArticleDto> getArticleById(@PathVariable("id") long id) {
        ResponseResult<ArticleDto> result = new ResponseResult<ArticleDto>();

        ArticleDto article = articleService.findArticleByIdWithComments(id);
        //判断文章是否存在
        if (article != null) {
            result.setStatus(0);
